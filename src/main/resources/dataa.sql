...
",
        NOW()),
       (1, "올림픽 시나리오", "너는 올림픽의 역사를 토론하는 자리의 진행자 역할을 맡았어. 1단계에는 참여자들이 서로 얘기를 하도록 해야해",
        "
phase(1) {
    let_avatar_speak(gpt_query('사람들에게 약 30초 분량으로 인사해. 그 후에 1단계 안내 및 참여자들이 서로 얘기해야 하는 내용들을 설명해줘.'));
}
        ...
        let_avatar_speak(
            gpt_query(
                string_concat(
                    ...
                    '해당 참여자에 대한 피드백을 약 20초 분량으로 진행해줘.'
                )
            )
        );
    }
